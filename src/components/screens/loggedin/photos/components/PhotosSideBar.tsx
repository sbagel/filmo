...
function CategoryFormat({title, setCategory, category}){
  return (
    ...
    :
    <div onClick={() => setCategory(title)} className='flex justify-center font-semibold border-y-[.2px] border-placeholder/[0.2] cursor-pointer hover:bg-cornflower/[.2]  transition-all delay-[30ms] ease-in-out'>
    {title}
    </div>
  )
